...
class Generator():
    ...
    def sample_to_image(self, samples):
        ...
        # if(init_image is not None and init_alpha is not None):
        #     from ipdb import set_trace as bb
        #     x_samples = x_samples * init_alpha + init_image * (1 - init_alpha)

        x_samples = torch.clamp((x_samples + 1.0) / 2.0, min=0.0, max=1.0)
        if len(x_samples) != 1:
            raise Exception(
                f'>> expected to get a single image, but got {len(x_samples)}')
            # x_samples = self.apply_mask(x_samples, init_image, init_mask)
